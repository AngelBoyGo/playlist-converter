...
from backend.app.services.playlist_scraper import (
    PlaylistScraper,
...
async def convert_playlist_data(
    converter: PlaylistConverter, playlist_data: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        success_count = sum(
            1
            for track in converted_data.get("tracks", [])
            if track.get("success", False)
        )
        logger.info("\nConversion Results:")
...
async def test_playlist_conversion():
    ...
    try:
        # Test playlist URL
        playlist_url = (
            "https://music.apple.com/us/playlist/levitated/pl.u-vxy6696sz1VKqBX"
        )
        ...
        logger.info(
            f"Successfully fetched playlist: {playlist_data.get('name', 'Unknown')}"
        )
        logger.info(f"Total tracks: {len(playlist_data.get('tracks', []))}")
        ...
        if playlist_data.get("tracks"):
            ...
            logger.info(
                f"Testing conversion with first track: {track_name} by {artist_name}"
            )
            ...
            if soundcloud_track:
                logger.info(
                    f"Successfully found match on SoundCloud: {soundcloud_track.get('title')}"
                )
            else:
