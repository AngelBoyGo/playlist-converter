...
class CircuitBreaker:
    ...
    def record_success(self):
        ...
        if self.state == "HALF-OPEN":
            self.state = "CLOSED"
            logger.info(
                "Circuit breaker reset to CLOSED state after successful test call"
            )
    ...
    def can_execute(self) -> bool:
        ...
        if self.state == "OPEN":
            ...
            cooling_time_elapsed = (
                time.time() - self.last_failure_time >= self.cooling_period
            )
            if cooling_time_elapsed:
                self.state = "HALF-OPEN"
                logger.info(
                    f"Circuit switched to HALF-OPEN state after {self.cooling_period}s cooling period"
                )
                return True
            else:
...
async def retry_with_exponential_backoff(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(
                f"Attempt {attempt + 1} failed, retrying in {delay:.1f}s", exc_info=True
            )
