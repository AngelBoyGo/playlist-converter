...
def create_error_response(
    message: str, request_id: str = None, progress: Dict = None
) -> ConversionResponse:
...
async def convert_playlist(
    ...
    logger.info(
        f"[TRACE][{request_id}] Received conversion request for URL: {request.url}"
    )
    ...
    try:
        ...
        update_progress(
            response, "initialization", "Initializing playlist scraper...", "starting"
        )
        ...
        update_progress(
            response, "initialization", "Initializing services...", "setting_up"
        )
        ...
        logger.info(
            f"[TRACE][{request_id}] SoundCloud service initialized successfully in {init_time:.2f}s"
        )

    except Exception as e:
        logger.error(
            f"[ERROR][{request_id}] Failed to initialize services: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to initialize services: {str(e)}"
        )

    # Fetch playlist data
    try:
        update_progress(
            response, "fetching_playlist", "Fetching playlist data", "in_progress"
        )
        ...
        async with timeout_context(
            180
        ):  # Increased timeout for playlist fetch to 3 minutes
            logger.info(
                f"[TRACE][{request_id}] Fetching playlist data from {request.url}"
            )
            playlist_data = await playlist_scraper.get_playlist_data(request.url)

        playlist_fetch_time = time.time() - playlist_fetch_start
        response.details.performance_stats["playlist_fetch_time"] = round(
            playlist_fetch_time, 2
        )
        ...
        logger.error(
            f"[ERROR][{request_id}] Playlist fetch timed out after 180 seconds"
        )
        update_progress(
            response,
            "error",
            "Error: Playlist fetch timed out after 180 seconds. Try again or use a smaller batch size.",
            "error",
        )
        ...
        logger.error(
            f"[ERROR][{request_id}] Failed to fetch playlist data: {str(e)}",
            exc_info=True,
        )
    ...
    update_progress(
        response,
        "tracks_found",
        f"Found {len(tracks)} tracks in playlist",
        "in_progress",
    )
    logger.info(f"[TRACE][{request_id}] Found {len(tracks)} tracks in playlist")
    ...
    try:
        ...
        for i, track in enumerate(tracks, start=request.start_index):
            try:
                ...
                update_progress(
                    response,
                    "processing_batch",
                    f"Processing batch {progress['current_batch']} of {progress['total_batches']} ({track_number}/{len(tracks)}): '{track_name}' by {artist_str}",
                    "in_progress",
                )

                logger.info(
                    f"[TRACE][{request_id}] Processing track {i+1}/{len(tracks)}: '{track_name}' by {artist_str}"
                )
                ...
                try:
                    ...
                    if sc_track:
                        update_progress(
                            response,
                            "track_found",
                            f"Found match for '{track_name}' in {search_time:.2f}s",
                            f"Found in {search_time:.2f}s",
                        )
                        logger.info(
                            f"[TRACE][{request_id}] Found match: '{sc_track.get('title')}' by {sc_track.get('user', {}).get('username')} in {search_time:.2f}s"
                        )
                        ...
                        logger.info(
                            f"[TRACE][{request_id}] Successfully processed track {i+1}"
                        )
                    else:
                        ...
                        update_progress(
                            response,
                            "track_not_found",
                            f"No match found for '{track_name}' in {search_time:.2f}s",
                            msg,
                        )
                        logger.warning(f"[WARN][{request_id}] {msg}")

                        track_result = TrackResult(
                            source_track=track, success=False, message=msg
                        )
                    ...
                    update_progress(
                        response,
                        "track_timeout",
                        f"Search timed out for '{track_name}' after 30s",
                        msg,
                    )
                    logger.warning(f"[WARN][{request_id}] {msg}")
                    ...
                    try:
                        logger.info(
                            f"[TRACE][{request_id}] Attempting browser recovery after timeout"
                        )
                        ...
                        await soundcloud.initialize_browser()
                        logger.info(
                            f"[TRACE][{request_id}] Successfully reset browser after timeout"
                        )
                    except Exception as e:
                        logger.error(
                            f"[ERROR][{request_id}] Failed to reset browser: {str(e)}"
                        )
                ...
                if tracks_processed > 0:
                    elapsed_time = (
                        datetime.now() - progress["batch_start_time"]
                    ).total_seconds()
                    time_per_track = elapsed_time / tracks_processed
                    tracks_remaining = (
                        progress["tracks_in_current_batch"] - tracks_processed
                    )
                    ...
                    if conversion_stats["perf_stats"]["search_times"]:
                        conversion_stats["perf_stats"][
                            "avg_search_time"
                        ] = conversion_stats["perf_stats"]["total_search_time"] / len(
                            conversion_stats["perf_stats"]["search_times"]
                        )
                ...
                try:
                    ...
                    if stats.get("rate_limiter", {}).get("limited_requests", 0) > 0:
                        ...
                        update_progress(
                            response,
                            "rate_limited",
                            "Hit rate limit - searches may be slower",
                            "Hit rate limit - searches may be slower",
                        )
                except:
                    pass
                ...
                logger.error(
                    f"[ERROR][{request_id}] Error processing track: {str(e)}",
                    exc_info=True,
                )
                update_progress(
                    response,
        ...
        update_progress(
            response,
            "batch_complete",
            f"Completed batch {progress['current_batch']} of {progress['total_batches']}. Found {response.success_count} of {len(tracks)} tracks.",
            f"Completed batch {progress['current_batch']} of {progress['total_batches']}. Found {response.success_count} of {len(tracks)} tracks.",
        )
        ...
        response.details.success_rate = (
            response.success_count / response.details.total_tracks
            if response.details.total_tracks > 0
            else 0
        )
        ...
        logger.error(
            f"[ERROR][{request_id}] Conversion process failed: {str(e)}", exc_info=True
        )
        update_progress(
            response,
            "error",
            f"Conversion process failed: {str(e)}",
            f"Conversion process failed: {str(e)}",
        )
        response.success = False
...
async def search_track(request: SearchRequest):
    ...
    try:
        ...
        result = await sc_service.search_track(
            request.track_name, request.artist_name, request.blacklisted_urls
        )
